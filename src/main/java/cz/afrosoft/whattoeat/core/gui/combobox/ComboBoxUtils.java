...
public final class ComboBoxUtils {
    ...
    /**
     * Initializes check combo box so it displays message defined by key of {@link Labeled} items instead of default toString
     * result.
     *
     * @param checkComboBox (NotNull) Check combo box to init.
     * @param <T>           Type of combo box item implementing {@link Labeled}.
     */
    public static <T extends Labeled> void initLabeledCheckComboBox(final CheckComboBox<T> checkComboBox) {
        Validate.notNull(checkComboBox);

        checkComboBox.setConverter(createStringConverter(checkComboBox, labeled -> I18n.getText(labeled.getLabelKey())));
    }
    ...
    /**
     * Creates string converter for specified check combo box which uses supplied map function to do conversion.
     *
     * @param comboBox    (NotNull) Check combo box for which converter is created.
     * @param mapFunction (NotNull) Function which maps item from combo box to String.
     * @param <T>         Type of items in combo box.
     * @return (NotNull) New converter based on map function.
     */
    public static <T> StringConverter<T> createStringConverter(final CheckComboBox<T> comboBox, final Function<T, String> mapFunction) {
        Validate.notNull(comboBox);
        Validate.notNull(mapFunction);
